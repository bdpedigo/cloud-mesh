...
post_prediction_path = Path(
    f"gs://bdp-ssa/{DATASTACK}/{PARAMETER_NAME}/{VERSION}/{MODEL_NAME}-{MODEL_VARIANT}/post-synapse-predictions"
)
...
def run_batch(i):
    ...
    put_dataframe(
        posteriors, post_prediction_path / f"{i}.csv.gz", compression="gzip"
    )
